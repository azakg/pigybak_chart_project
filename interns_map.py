...
GEO_CACHE   = "geo_cache.csv"      # автокэш геокодинга
OVERRIDES_CSV = "overrides.csv"    # (необязательно) ручные соответствия: University / School,lat,lon,state
...
def _read_table(src: str, sheet: Optional[str]) -> pd.DataFrame:
    ...
    if sl.startswith(("http://", "https://")):
        r = requests.get(src, headers={"User-Agent": "Mozilla/5.0"}, timeout=30, verify=certifi.where())
        r.raise_for_status()
...
def load_overrides(path: str) -> Dict[str, Tuple[float, float, str, str]]:
    """Ручные переопределения: University / School,lat,lon,state (двухбуквенно) — country заполним 'United States' если есть state."""
    try:
        df = pd.read_csv(path)
        out = {}
        for _, r in df.iterrows():
            ...
            country = "United States" if st in STATE_ABBR.values() else str(r.get("country", "")).strip()
...
def geocode_university(univ: str, geocode_fn, cache: Dict[str, Tuple[float, float, str, str]],
    ...
    # 2) каскад запросов (таймауты увеличены)
    queries = [
        f"{key}, United States",
        f"{key}",
        f"{key}, Canada",
...
def build_map(df_state: pd.DataFrame, df_unis: pd.DataFrame) -> folium.Map:
    ...
    max_count = int(df_state["count"].max()) if not df_state.empty else 1
    ...
    for _, r in df_state.iterrows():
        folium.Circle(
            ...
            tooltip=f"{r['state']}: {r['count']} intern(s)",
            color=BUBBLE_HEX,
            fill=True, fill_opacity=0.35, fill_color=BUBBLE_HEX, weight=2,
        ).add_to(layer_states)

    # Слой универов (кластер)
    layer_unis = folium.FeatureGroup(name="Universities", show=True); fmap.add_child(layer_unis)
    cluster = MarkerCluster().add_to(layer_unis)

    for _, r in df_unis.iterrows():
        popup = folium.Popup(
            html=f"<b>{r['university']}</b><br>{r['count']} intern(s)<br><br>" + "<br>".join(r["names"]),
            max_width=360
        )
        folium.CircleMarker(
            location=[float(r["lat"]), float(r["lon"])],
            radius=6 if r["count"] == 1 else min(12, 6 + r["count"]),
            weight=1, color="#1f2937",
            fill=True, fill_color=UNI_MARKER_COLOR, fill_opacity=0.9,
            tooltip=f"{r['university']}: {r['count']} intern(s)",
            popup=popup,
        ).add_to(cluster)
...
def main():
    ...
    needed = [FIRST_NAME_COL, LAST_NAME_COL, UNIVERSITY_COL, LOCATION_COL, ACCEPT_DATE_COL]
    missing = [c for c in needed if c not in df.columns]
    if missing:
        raise SystemExit(f"Нет колонок: {missing}")
    ...
    # Геокодер с увеличенным таймаутом и ретраями
    # Геокодер с увеличенным таймаутом и ретраями
    geolocator = Nominatim(
        user_agent="interns-bubble-map",
        ssl_context=SSL_CTX,
        scheme="https",
    ...
    for univ, loc_fallback in zip(df[UNIVERSITY_COL], df[LOCATION_COL]):
        ...
        # если штат не определился, пробуем из колонки Location
        if not st:
            st = parse_state_from_text(loc_fallback)
        lat_list.append(lat)
        lon_list.append(lon)
        st_list.append(st)
    ...
    df["full_name"] = (df[FIRST_NAME_COL].fillna("").astype(str).str.strip()
    ...
    # Агрегат по штату ТОЛЬКО для США
    ...
    # Агрегат по университетам (все страны) с именами
    grouped_uni = (
        df_pts.groupby([UNIVERSITY_COL])
              .agg(
                  ...
                  names=("full_name", lambda s: list(sorted(s)))
              )
              .reset_index()
